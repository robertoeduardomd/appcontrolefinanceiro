//construir o funcionamento do pop up
...
//declaração de variaveis coletando ids e classes
...
const addtransacaonoDOM = (transacao) => {
  ...
  li.innerHTML = ` <span>
 <p> Nome objeto:</p> ${transacao.name}    <p> Data:</p> ${transacao.data} <p>Valor:</p>  ${operacao} R$ ${montantesemoperador} </span>
  <button class="delete-btn" onClick="removertransacao(${transacao.id})" >x</button>
  `;
  transacoesUL.prepend(li);
  //fechar apos adicionar um objeto:
  // modal.classList.remove("abrir");
};
